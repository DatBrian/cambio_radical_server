...
class App extends ConnectionDB_1.ConnectionDB {
    ...
    async initConnection() {
        try {
            ...
            console.log(chalk_1.default.green.bold(`🌐 ¡Se ha establecido la conexión a: ${process.env.DB_NAME}!`));
            console.log(chalk_1.default.blue("---------------------------------------------------------------------------------"));
            return connection;
        }
        catch (error) {
    ...
    initRoutes(routes) {
        this.app.use(`/api/${process.env.API_VERSION}`, AuthRoutes_1.authRoutes.router);
        routes.forEach((route) => {
            this.app.use(`/api/${process.env.API_VERSION}`, AuthJWTMiddleware_1.authJWTMiddleware.validateToken, route.router);
        });
    }
    listen() {
        this.app.listen(this.port, () => {
            ...
            console.log(chalk_1.default.blue("--------------------------------------------------------------------------------"));
            console.log(chalk_1.default.green.bold(`🚀 ¡El servidor se ha levantado exitosamente en http://${process.env.HOST}:${process.env.PORT}!`));
            console.log(chalk_1.default.blue("--------------------------------------------------------------------------------"));
        });
    }
}
